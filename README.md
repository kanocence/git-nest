...
- **仓库级 FIFO 任务队列**：同一仓库可同时提交多个任务，系统自动按提交顺序串行执行，队列状态在 `/tasks` 页面实时可见
...
- **占用释放**：对 `running`/`queued`/`preparing` 状态的 run 提供 Release 操作；`queued` run 直接取消并自动触发队列中下一个任务，其他状态发送取消信号并释放工作区锁
