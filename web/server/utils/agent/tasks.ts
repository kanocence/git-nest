import type { TaskDefinition, TaskDefinitionV2, TaskEdge, TaskNode, TaskSummary } from '#shared/types/agent'
...
function parseTaskDefinitionUnsafe(content: string, filePath: string): TaskDefinition {
  ...
  if (document.errors.length > 0) {
    throw new AgentError(422, 'INVALID_TASK_YAML', 'Task YAML is invalid', {
      path: filePath,
  ...
  if (!isPlainObject(raw)) {
    throw new AgentError(422, 'INVALID_TASK_YAML', 'Task YAML root must be an object', {
      path: filePath,
    })
  }
...
export function parseTaskDefinition(content: string, filePath: string): TaskDefinition {
  const definition = parseTaskDefinitionUnsafe(content, filePath)
  if (!definition.valid) {
    throw new AgentError(422, 'INVALID_TASK_YAML', 'Task YAML failed validation', {
      path: filePath,
...
export function parseTaskDefinitionV2(content: string, filePath: string): TaskDefinitionV2 {
  ...
  if (version === 1) {
    console.warn('[tasks] Deprecated v1 format detected, auto-migrating', { filePath })
